...
/** Background-only layer: grid + optional cursor-reveal mask + orbs. No foreground content. */
export function InfiniteGridBackground({
  ...
  const hasMouse = externalMouseX != null && externalMouseY != null;
  const mouseX = externalMouseX ?? fallbackX;
  const mouseY = externalMouseY ?? fallbackY;
  const maskImage = useMotionTemplate`radial-gradient(300px circle at ${mouseX}px ${mouseY}px, black, transparent)`;
    ...
    >
      <div className="absolute inset-0" style={{ opacity: gridOpacity }}>
        <GridPattern
          offsetX={gridOffsetX}
          offsetY={gridOffsetY}
          patternId="grid-pattern-bg"
          strokeClassName="text-neutral-400"
        />
      </div>
      {hasMouse && (
        <motion.div
        ...
        >
          <GridPattern
            offsetX={gridOffsetX}
            offsetY={gridOffsetY}
            patternId="grid-pattern-bg-mask"
            strokeClassName="text-neutral-400"
          />
        </motion.div>
      )}
