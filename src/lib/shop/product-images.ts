...
export async function uploadShopProductImage(
  ...
  const { error: upErr } = await admin.storage.from(SHOP_PRODUCT_IMAGES_BUCKET).upload(objectPath, buffer, {
    contentType,
    upsert: false,
  })
  if (upErr) return { error: upErr.message }
  const { data } = admin.storage.from(SHOP_PRODUCT_IMAGES_BUCKET).getPublicUrl(objectPath)
