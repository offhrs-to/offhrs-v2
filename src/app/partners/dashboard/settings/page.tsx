...
export default async function SettingsPage() {
  ...
  const { data: vendor } = await admin
    ...
    .select(
      'id, business_name, bio, website_url, instagram_handle, phone, location_address, location_unit, refund_window_hours, strict_no_refund, status, subscription_current_period_end, stripe_customer_id, gst_hst_registered, gst_hst_registration_number'
    )
    .eq('user_id', user.id)
    .single()
  ...
  const { data: subscription } = hasNativePlan
    ? await admin
  ...
  if (hasNativePlan) {
    try {
      ...
      const stripeSubscriptionId =
        (subscription?.stripe_subscription_id as string | null | undefined) ?? null
